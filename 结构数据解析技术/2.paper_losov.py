...
for station_index in range(station_number):
    ...
    metrics[0, 0], metrics[0, 1] = models[0].score(train_x, train_y, train_y_scaler, print_=False, plot_=False)   # 评价模型
    metrics[0, 2], metrics[0, 3] = models[0].score(val_x, val_y, val_y_scaler, print_=False, plot_=False)   # 评价模型
    # 6.2 训练SVR模型
    models[1] = bias_correction_model(method='SVR')  # 模型初始化
    models[1].fit(train_x, train_y.reshape(-1, ))    # 训练
    metrics[1, 0], metrics[1, 1] = models[1].score(train_x, train_y, train_y_scaler, print_=False, plot_=False)   # 评价模型
    metrics[1, 2], metrics[1, 3] = models[1].score(val_x, val_y, val_y_scaler, print_=False, plot_=False)   # 评价模型
    # 6.3 训练ANN模型
    models[2] = bias_correction_model(method='ANN')  # 模型初始化
    models[2].fit(train_x, train_y.reshape(-1, ))    # 训练
    metrics[2, 0], metrics[2, 1] = models[2].score(train_x, train_y, train_y_scaler, print_=False, plot_=False)   # 评价模型
    metrics[2, 2], metrics[2, 3] = models[2].score(val_x, val_y, val_y_scaler, print_=False, plot_=False)   # 评价模型
    # 6.4 训练集成模型
    models[3] = bias_correction_ensemble_model(epochs=1, learning_rate=0.002)  # 模型初始化
    models[3].fit(train_x, train_y, show_train_process=False, print_w=False)      # 训练
    metrics[3, 0], metrics[3, 1] = models[3].score(train_x, train_y, train_y_scaler, print_=False, plot_=False)   # 评价模型
    metrics[3, 2], metrics[3, 3] = models[3].score(val_x, val_y, val_y_scaler, print_=False, plot_=False)   # 评价模型
    # 记录训练数据
    losov_info['models'].append(models)
    losov_info['metrics'].append(metrics)
    losov_info['model_names'].append(['RF', 'SVR', 'ANN', 'MME'])
...
# 7.模型评价
losov_print(losov_info)  # 打印结果
losov_plot(losov_info)   # 画图展示

# 保存指标值
metrics = np.array(losov_info['metrics']).reshape(-1, 4)
with open('Results/losov_metrics.csv', 'w') as datafile:
