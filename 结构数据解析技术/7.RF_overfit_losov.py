...
for para in paras:
    ...
    for station_index in range(station_number):
        ...
        print('Training RF model：%d/25 ' % (station_index+1))
    print('训练完成！\n')
...
with open('Results/RF_losov_logs', 'w') as datafile:
...
plt.title('R_square')
plt.legend()
plt.savefig(r'Figs/losov：R_square随参数变化的曲线.png')
plt.show()
...
plt.title('RMSE')
plt.legend()
plt.savefig(r'Figs/losov：RMSE随参数变化的曲线.png')
plt.show()
