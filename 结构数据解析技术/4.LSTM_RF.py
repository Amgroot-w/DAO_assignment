...
model1 = LSTM_DAE(input_num=21, hidden_num=10, output_num=21,
                  time_step=10, batch_size=60, learning_rate=0.01,
                  lamda=0.00001, dropout_keep_prob=1, epochs=500)
for station_index in range(station_number):
    ...
    # 6.训练
    model1.train(train_x, val_x)

    # 7.获取特征
    train_feature = model1.get_feature(train_x)
    val_feature = model1.get_feature(val_x)

    # %% 8.训练RF回归模型
    model2 = RandomForestRegressor(n_estimators=100, min_samples_leaf=1)
    model2.fit(train_feature, train_y)  # 训练

    # 预测
    train_pred = model2.predict(train_feature).reshape(-1, 1)
    val_pred = model2.predict(val_feature).reshape(-1, 1)

    # 反归一化
    train_pred_scale = val_y_scaler.inverse_transform(train_pred)
    train_y_scale = val_y_scaler.inverse_transform(train_y)
    val_pred_scale = val_y_scaler.inverse_transform(val_pred)
    val_y_scale = val_y_scaler.inverse_transform(val_y)

    # 9.评价拟合效果
    R_square1 = r2_score(train_y_scale, train_pred_scale)  # 决定系数
    RMSE1 = mean_squared_error(train_y_scale, train_pred_scale, squared=False)  # 均方根误差
    R_square2 = r2_score(val_y_scale, val_pred_scale)  # 决定系数
    RMSE2 = mean_squared_error(val_y_scale, val_pred_scale, squared=False)  # 均方根误差

    # 记录指标值
    metrics.append([R_square1, RMSE1, R_square2, RMSE2])
    # 打印训练结果
    print('Training "LSTM+RF" model：%d/25 ' % (station_index+1))
print('训练完成！\n')

# %% 展示结果
metrics = np.array(metrics)
# 保存指标值
with open('Results/lstm_rf_metrics.csv', 'w') as datafile:
    writer = csv.writer(datafile, delimiter=',')
    writer.writerows(metrics)
print('LSTM+RF模型的指标平均值:   训练集: R_square: %.4f, RMSE: %.4f;   验证集: R_square: %.4f, RMSE: %.4f'
